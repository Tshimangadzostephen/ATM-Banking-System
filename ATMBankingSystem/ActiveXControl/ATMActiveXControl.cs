...
namespace ATMBankingSystem.ActiveXControl
{
    ...
    public class ATMActiveXControl : IATMActiveXControl
    {
        ...
        public bool SetAmount(decimal amount)
        {
            try
            {
                LogMessage("Amount set: R" + amount);
                
                // Check limits
                if (amount > 500)
                {
                    lastError = "Amount exceeds daily limit of R500";
                    LogMessage("ERROR: " + lastError);
                    return false;
                }
        ...
        {
            LogMessage("Balance inquiry: R" + accountBalance);
        ...
        public bool ProcessTransaction()
        {
            try
            {
                LogMessage("Processing " + selectedTransaction + " for R" + transactionAmount);
                ...
                if (selectedTransaction == "WITHDRAWAL")
                {
                    if (transactionAmount <= accountBalance)
                    {
                        ...
                        LogMessage("Withdrawal successful. New balance: R" + accountBalance);
                        return true;
                    }
                    else
                    {
        ...
        {
            var subKey = Registry.ClassesRoot.OpenSubKey(key.Replace(@"HKEY_CLASSES_ROOT\", ""), true);
            if (subKey != null)
            {
